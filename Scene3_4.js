...
class Scene3_4 extends Phaser.Scene
{
    constructor()
    {
        super("Scene3_4");
    ...
    create ()
    {
        ...
        var stump1 = trees.create(800,300,'bigStump');
        stump1.body.setOffset(0, -20);
        stump1.setScale(2);
        stump1.body.setCircle(60);

        var blood = this.add.sprite(735,425,'blood');
        blood.setFrame(5);
        blood.flipY = true;
        blood = this.add.sprite(665,400,'blood');
        blood.setFrame(5);
        blood = this.add.sprite(690,260,'blood');
        blood.setFrame(5);
        blood = this.add.sprite(650,315,'blood');
        blood.setFrame(2);
        blood.flipY = true;
        this.add.sprite(770,270,'blood').setScale(1.3);

        var dirt = this.add.image(150,180,'dirt').setScale(1.2);
        dirt.flipX = true;
        dirt.flipY = true;
        this.add.image(140,160,'dirt').setScale(1.4);
        dirt = this.add.image(380,430,'dirt').setScale(1.2);
        dirt.flipX = true;

        var grass = this.add.image(590,175,'grass');
        grass.flipX = true;
        this.add.image(460,260,'grass');
        grass = this.add.image(530,525,'grass');
        grass.flipY = true;
        grass = this.add.image(230,350,'grass');
        grass.flipX = true;
        grass.flipY = true;
        this.add.image(60,300,'grass');
        ...
        // room change objects
        bottomBorder = this.add.rectangle(450,600,280,25, 0xfcf488,.4);
        this.physics.add.existing(bottomBorder);
        this.physics.add.overlap(player, bottomBorder, this.moveRoomDown, null, this);

        leftBorder = this.add.rectangle(0,300,25,200, 0xfcf488,.4);
        this.physics.add.existing(leftBorder);
        this.physics.add.overlap(player, leftBorder, this.moveRoomLeft, null, this);

        rightBorder = this.add.rectangle(800, 170, 25, 100, 0xfcf488,.4);
        this.physics.add.existing(rightBorder);
        this.physics.add.overlap(player, rightBorder, this.moveRoomRight, null, this);

        var rightBorder2 = this.add.rectangle(800, 400, 25, 100, 0xfcf488,.4);
        this.physics.add.existing(rightBorder2);
        this.physics.add.overlap(player, rightBorder2, this.moveRoomRight, null, this);
    ...
    {
        prevRoom = "Scene3_4";
        this.scene.start("Scene4_4", {x: playerX, y: 550});
    }

    moveRoomDown(player, bottomBorder)
    {
        prevRoom = "Scene3_4";
        this.scene.start("Scene2_4", {x: playerX, y: 50});
    }

    moveRoomLeft(player, leftBorder)
    {
        prevRoom = "Scene3_4";
        this.scene.start("Scene3_3", {x: 750, y: playerY});
    }

    moveRoomRight(player, rightBorder)
    {
        prevRoom = "Scene3_4";
        this.scene.start("Scene3_5", {x: 50, y: playerY});
    }
};
